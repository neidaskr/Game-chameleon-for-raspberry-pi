...
def start_real_game():
    ...
    # Užtikriname, kad sąrašų ilgiai sutaptų (nukerpame ilgesnį, bet nieko neišimame iš kodo)
    if len(zodziu_sarasas) != len(chameleon_zodziai):
        ...
        min_len = min(len(zodziu_sarasas), len(chameleon_zodziai))
        zodziu_sarasas = zodziu_sarasas[:min_len]
        chameleon_zodziai = chameleon_zodziai[:min_len]
